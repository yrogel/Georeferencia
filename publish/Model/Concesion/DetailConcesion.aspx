...
<asp:Content runat="server" ID="BodyContent" ContentPlaceHolderID="MainContent">
    ...
    <script>
        ...
        function initializeLinea(coordenadasEValue) {
            //console.log(coordenadasEValue);
            //var e = new Object();
            
            for (var i = 0; i < coordenadasEValue.length; i += 2) {
                //e.lat = coordenadasEValue[i]["lat"];
                //e.lng = coordenadasEValue[i]["lng"];
               
                var linea = [];
                linea[0] = coordenadasEValue[i];
                linea[1] = coordenadasEValue[i + 1];
               
                //console.log(linea);
               
                var lineaMap = new google.maps.Polyline({
                    path: linea,
                    geodesic: true,
                    strokeColor: '#FF0000',
                    strokeOpacity: 1.0,
        ...
        function OnSuccess(data) {
            ...
            if (coordenadas.length > 0) {
                ...
                $.ajax({
                    type: "POST",
                    url: '<%= Page.ResolveUrl("~/Model/Concesion/DetailConcesion.aspx/ObtenerCoordenadasLineas")%>',
                    data: JSON.stringify(obj),
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: OnSuccessCoordenadasLineas,
                    error: OnErrorCoordenadasLineas
                });
        ...
        function OnSuccessCoordenadasLineas(data) {
            //console.log(data.d);
            //console.log(data);
            ...
            //alert(coordenadas.length);
            if (coordenadas.length > 0) {
                var coordenadasEValue = [];
                var j = 0;
                for (var i = 0; i < coordenadas.length; i++) {
                    //console.log('Id: ' + coordenadas[i]["Id"] + ' - UtmEInicio: ' + coordenadas[i]["UtmEInicio"] + ' - UtmEFin: ' + coordenadas[i]["UtmEFin"] + ' - UtmNInicio: ' + coordenadas[i]["UtmNInicio"] + ' - UtmNFin: ' + coordenadas[i]["UtmNFin"]);
                    coordenadasEValue[j] = UtmToLongLat(coordenadas[i]["UtmEInicio"], coordenadas[i]["UtmEFin"]);
                    coordenadasEValue[j + 1] = UtmToLongLat(coordenadas[i]["UtmNInicio"], coordenadas[i]["UtmNFin"]);
                    //console.log(UtmToLongLat(coordenadas[i]["UtmEInicio"], coordenadas[i]["UtmEFin"]));
                    //console.log(UtmToLongLat(coordenadas[i]["UtmNInicio"], coordenadas[i]["UtmNFin"]));
                    j += 2;
                }
                //console.log(coordenadasEValue);
                initializeLinea(coordenadasEValue);
            } else {
                $("#<%= lblError.ClientID %>").text("La concesión no tiene lineas configuradas para visualizar su ubicación.");
                $("#<%= mensaje.ClientID %>").addClass("alert alert-info fade in");
                $("#<%= mensaje.ClientID %>").css('display', 'block');
            }
        }

        function OnErrorCoordenadasLineas(data) {
            //alert('error');
            //alert(data.d);
            console.log(data);
        }
